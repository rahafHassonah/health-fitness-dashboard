    ...
    tabItems(
      ...
      tabItem(
        ...
        tabsetPanel(
          ...
          tabPanel("Body Composition Performance",
                   br(),
                   box(
                     title = "Body Composition Performance Matrix",
                     status = "primary",
                     solidHeader = TRUE,
                     width = 12,
                     plotOutput("body_comp_heatmap")
                   )
          ),
...
server <- function(input, output, session) {
  ...
  # =====================================
  # BODY COMPOSITION LOLLIPOP
  # =====================================
  output$body_comp_heatmap <- renderPlot({
    body_comp_lollipop <- data %>%
      mutate(
        BMI_category = cut(BMI, 
                           breaks = c(0, 18.5, 25, 30, 100),
                           labels = c("Underweight", "Normal", "Overweight", "Obese"),
                           include.lowest = TRUE),
        Fat_category = cut(Fat_Percentage, 
                           breaks = c(0, 55, 60, 65, 70, Inf),
                           labels = c("Low", "Moderate", "Mod-High", "High", "Very High"),
                           include.lowest = TRUE),
        body_comp = paste0(BMI_category, "\n", Fat_category)
      ) %>%
      filter(!is.na(BMI_category) & !is.na(Fat_category) & !is.na(Workout_Type)) %>%
      group_by(body_comp, Workout_Type, BMI_category, Fat_category) %>%
      summarise(
        avg_calories = mean(Calories_Burned, na.rm = TRUE),
        avg_hr = mean(Avg_BPM, na.rm = TRUE),
        count = n(),
        .groups = "drop"
      ) %>%
      filter(count > 3) %>%
      group_by(Workout_Type) %>%
      mutate(rank = rank(-avg_calories)) %>%
      ungroup()
    
    ggplot(body_comp_lollipop, aes(x = reorder(body_comp, avg_calories), 
                                   y = avg_calories, color = BMI_category)) +
      geom_segment(aes(xend = body_comp, yend = 0), size = 1.5, alpha = 0.7) +
      geom_point(size = 5, alpha = 0.9) +
      geom_text(aes(label = round(avg_calories, 0)), 
                hjust = -0.3, size = 2.8, fontface = "bold", color = "black") +
      facet_wrap(~ Workout_Type, ncol = 2, scales = "free_y") +
      scale_color_brewer(palette = "RdYlGn", direction = -1, name = "BMI Category") +
      coord_flip() +
      labs(title = "Body Composition Performance Rankings",
           subtitle = "Ranked by average calorie burn | Lower BMI categories dominate top positions",
           x = "Body Composition (BMI + Fat %)",
  ...
  # =====================================
  # NEW: PLOT B - Workout Performance Over Experience
  # =====================================
  ...
  # =====================================
  # NEW: PLOT J - Sweet Spot Contour Density
  # =====================================
